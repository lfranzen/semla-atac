...
LoadAndProcessCoords <- function(
  ...
  #  }
    ...
    # Initialize an empty vector to collect missing file paths
    missing_files <- character()
    
    # Specify the columns you want to check for file existence
    columns_to_check <- c("fragments", "spotfiles")
    if ("json" %in% colnames(infoTable)) {
      columns_to_check <- c(columns_to_check, "json")
    }
    
    # Loop through each specified column and check if the files exist
    for (col_name in columns_to_check) {
      paths <- infoTable[[col_name]]
      missing <- paths[!file.exists(paths)]
      if (length(missing) > 0) {
        missing_files <- c(missing_files, missing)
      }
    }
    
    # Abort the function if there are any missing files and report them
    ...
    if ("scalefactor" %in% colnames(infoTable)) {
      ...
      # Add full res image dimensions to image data
      #scalefactors <- infoTable$scalefactors
